...
class CarSearch {
  ...
  async searchCars(params: SearchParams) {
    ...
    if (carNames.length) {
      const createdCarSelector = carNames
        .map((car) => `:contains("${car}")`)
        .join(", ");
      resultElem = resultElem.has(
        `div.service-item__info__car-type > p:not(.label-sp)${createdCarSelector}`,
      );
    }
