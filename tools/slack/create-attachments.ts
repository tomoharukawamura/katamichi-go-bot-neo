...
const extractShopData = (shopStr: string): ShopData => {
  ...
  const areaName = isSingle
    ? detailedArea!
    : area.concat(detailedArea ? `(${detailedArea})` : "");
...
export const createAttachments = (car: CarWithType, idx = 0) => {
  ...
  const { area: returnAreaName, link: returnAreaLink } = extractShopData(
    car.returnShop,
  );
    ...
    fields: [
      ...
      },
      {
        ...
        value: car.returnShop.concat(
          car.type !== "soldOut"
            ? `（返却可能店舗は<${returnShopCandidateLink}|こちら>）`
            : "",
        ),
        short: false,
      },
