...
const EXECUTION_TIMES = 
  process.env.EXECUTION_TIMES
  ? parseInt(process.env.EXECUTION_TIMES, 10)
  : 1
const EXECUTION_INTERVAL_SECONDS =
  process.env.EXECUTION_INTERVAL_SECONDS
...
export const handler = async () => {
  ...
  for (let i = 0; i < EXECUTION_TIMES; i++) {
    ...
    if (errors.length) {
      console.error(`[Run ${i + 1}/${EXECUTION_TIMES}] Failed to process some cars:`, errors.map((e) => e.reason))
